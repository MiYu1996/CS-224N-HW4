...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, batch_length = nn.utils.rnn.pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.permute(1,0,2)
        cat_hidden = torch.cat((last_hidden[1],last_hidden[0]),dim = 1)
        cat_cell = torch.cat((last_cell[1],last_cell[0]),dim = 1)
        dec_init_state = (self.h_projection(cat_hidden),self.h_projection(cat_cell))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        new_Y = torch.split(Y,1,dim = 0)
        for chunks in new_Y:
            Y_t = torch.squeeze(chunks,dim = 0)
